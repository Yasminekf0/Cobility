...
export default function PlanMode() {
  ...
  const s = {
    ...
    arrowIcon:{color:'#8899aa',fontSize:'14px'},
    stopCard:{background:'#1e2a3a',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'16px',padding:'16px 20px',marginBottom:'8px',overflow:'hidden'},
    stopHeader:{display:'flex',justifyContent:'space-between',alignItems:'center'},
    ...
    streetView:{width:'100%',height:'180px',borderRadius:'12px',marginTop:'12px',border:'none'},
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.iconRow}>
        <span>🚇</span><span style={s.arrowIcon}>▶</span>
        <span>🛒</span><span style={s.arrowIcon}>▶</span>
        <span>🍴</span><span style={s.arrowIcon}>▶</span>
        <span>🚇</span><span style={s.arrowIcon}>▶</span>
        <span>🏛️</span>
        ...
        <div key={stop.id}>
          <div style={s.stopCard}>
            <div style={s.stopHeader}>
              ...
              <button 
                style={{...s.btn360, background: expanded===stop.id ? '#4A9EFF' : '#2a3a4a'}}
